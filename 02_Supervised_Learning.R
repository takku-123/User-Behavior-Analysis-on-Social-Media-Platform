...
data_long <- pivot_longer(data, cols = where(is.numeric), names_to = "Variable", values_to = "Value")
p <- ggplot(data_long, aes(x = Variable, y = Value)) +
  geom_boxplot() +
  theme_classic(base_size = 10) +  # Apply highcharter theme
  scale_colour_hc() +  # Apply highcharter color scale if this is desired
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 3),  
        plot.title = element_text(size = 10),  
        plot.margin = unit(c(1, 1, 1, 1), "cm")) + 
  labs(title = "               Boxplots for the dataset")
print(p)
...
ggplot(data, aes(x = AccountAge, y = TotalPosts, color = IsActive)) +
  geom_point() +
  scale_color_manual(values = c("blue", "red"), labels = c("Passive", "Active")) +
...
plot(data$TotalPosts, data$LikeRate, main="Total Posts vs. Like Rate",
     xlab="Total Posts", ylab="Like Rate", pch=19)+theme_minimal()
plot(data$AccountAge, data$PercBiNeighbours, main="Account Age vs. Percentage of Bidirectional Neighbours",
     xlab="Account Age", ylab="PercBiNeighbours", pch=19)+theme_minimal()
